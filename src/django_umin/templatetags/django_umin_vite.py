...
def _css_tag(url, preload=False):
    """Return a blocking or non-blocking CSS link tag."""
    if preload:
        return mark_safe(
...
@register.simple_tag
def vite_asset(path, app, preload=False):
    """
    Generates <script> or <link> tags for a Vite asset.
    Handles both development and production modes.
    Pass preload=True for CSS assets to emit a non-blocking preload link.
    """
    if VITE_DEV_MODE:
        ...
        if full_asset_url.endswith(".js"):
            ...
            return _css_tag(full_asset_url, preload)
    ...
    if asset_url.endswith(".js"):
        ...
        return _css_tag(asset_url, preload)
    else:
